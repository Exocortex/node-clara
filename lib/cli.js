...
function buildCommand(info, key, section) {
  ...
  var optionString = R.map(function(opt) {
    return '<' + opt + '>';
  }, urlParams).join(' ');
  ...
  cmd.action(function() {
    ...
    var queryArgs = R.mapObjIndexed(
      R.curry(convertParam)(query),
      R.pick(R.keys(query), cmd)
    );
    ...
    var opts =
      info.body && cmd.body
        ? cmd.body
        : R.mapObjIndexed(
            R.curry(convertParam)(cmdOptions),
            R.pick(R.keys(cmdOptions), cmd)
          );
    ...
    var fail = function(err, res) {
      if (err.status) {
        ...
        if (err.status === 401) {
          log.warn(
            'Have you set up your authentication properly?',
            'username: ',
            conf.get('username'),
            'apiToken: ',
            conf.get('apiToken')
    ...
    const queryOptions = Object.assign({}, args, queryArgs, { url: cmd.url });
    const outputFormat =
      typeof resource.output === 'function'
        ? resource.output(queryOptions, opts)
        : resource.output;

    claraApi[section][key](queryOptions, opts, function(err, result) {
      ...
      if (outputFormat === 'json') {
        if (program.jsonQuery) {
          ...
          output = JSON.stringify(
            jsonQuery(program.jsonQuery, { data: result }).value
          );
          if (output === undefined) {
      ...
      if (program.output) {
        fs.writeFileSync(
          program.output,
          output,
          outputFormat === 'binary' ? 'binary' : 'utf8'
        );
      } else {
